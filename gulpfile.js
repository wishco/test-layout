...
gulp.task('my1', function(done) {
	console.log("Привет, я таск!"); done();
});
...
gulp.task('prebuild2', async function() {
    ...
    .pipe(gulp.dest('dist/libs/normalize-css'))

    var buildFonts = gulp.src('app/fonts/**/*.otf') // Переносим шрифты в продакшен
    .pipe(gulp.dest('dist/fonts'))

    var buildJs = gulp.src('app/js/main.js') // Переносим скрипты в продакшен
    .pipe(gulp.dest('dist/js'))
...
gulp.task('watch', function() {
    gulp.watch('app/sass/**/*.sass', gulp.parallel('sass', 'code')); // Наблюдение за sass файлами
    gulp.watch('app/*.html', gulp.parallel('code', 'sass')); // Наблюдение за HTML файлами в корне проекта
    gulp.watch('app/**/*.php', gulp.parallel('codephp', 'sass')); // Наблюдение за PHP файлами в корне проекта
...
gulp.task('q1', gulp.parallel('sass', 'scripts', 'browser-sync', 'watch'));
gulp.task('build', gulp.parallel('prebuild', 'clean', 'img', 'sass', 'scripts'));


gulp.task('q2', gulp.parallel('sass', 'browser-sync', 'watch2'));
gulp.task('build2', gulp.parallel('prebuild2', 'clean', 'img', 'sass'));
